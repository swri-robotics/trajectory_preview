...
public:
  ...
  void initialize(const std::string &input_traj_topic,
  ...
  void onAnimateCallback(const ros::TimerEvent &e) {
    ...
    if (e.last_real ==
        ros::Time()) // Last time is zero if the timer gets restarted
    {
      step = 0;
    }
  ...
  void setCurrentTime(int index, int num_indexes) {
    ...
    current_time_ = total_time * static_cast<double>(index) /
                    static_cast<double>(num_indexes);
    publishStateAtTime(current_time_);
  }
